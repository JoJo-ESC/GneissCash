...
async function PATCH(request) {
    try {
        ...
        const { data: existing, error: fetchError } = await supabase.from('transactions').select('id').eq('id', id).eq('user_id', user.id).single();
        ...
        const { data: updated, error: updateError } = await supabase.from('transactions').update(updates).eq('id', id).select().single();
        if (updateError) {
...
async function DELETE(request) {
    try {
        ...
        // Verify transaction belongs to user
        const { data: existing, error: fetchError } = await supabase.from('transactions').select('id, import_id').eq('id', transactionId).eq('user_id', user.id).single();
        if (fetchError || !existing) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: 'Transaction not found'
            }, {
        ...
        if (existing.import_id) {
            const { data: currentImport } = await supabase.from('imports').select('transaction_count').eq('id', existing.import_id).single();
            if (currentImport && currentImport.transaction_count > 0) {
                await supabase.from('imports').update({
                    transaction_count: currentImport.transaction_count - 1
...
//# sourceMappingURL=%5Broot-of-the-server%5D__937d8ab7._.js.map